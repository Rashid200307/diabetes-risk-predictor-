...
MODEL_PATH = os.path.join(current_dir, 'models', 'random_forest.pkl')  # Updated to Random Forest
FEATURE_IMG_PATH = os.path.join(current_dir, 'models', 'feature_importance.png')

# --- ERROR HANDLING FOR DEPENDENCIES ---
try:
    # Import scikit-learn for Random Forest
    from sklearn.ensemble import RandomForestClassifier
except ImportError:
    st.error("""
    scikit-learn not installed! Please add 'scikit-learn==1.3.0' to requirements.txt.
    """)
    st.stop()
...
# --- MODEL PERFORMANCE METRICS (Updated with Random Forest metrics) ---
MODEL_PERFORMANCE = {
    "Accuracy": 0.8481,
    "F1 Score": 0.7692,
    "ROC AUC": 0.8890,
    "Precision": 0.75,  # Add your actual precision
    "Recall": 0.78      # Add your actual recall
}
...
with st.expander("About Our Model" if lang == "English" else "Tentang Model Kami", expanded=True):
    ...
    st.info("""
    Our best-performing model is Random Forest, which achieved 84.8% accuracy 
    and 88.9% AUC score in validation tests. It outperformed other models including 
    XGBoost and Logistic Regression.
    """ if lang == "English" else """
    Model terbaik kami adalah Random Forest, yang mencapai ketepatan 84.8% 
    dan skor AUC 88.9% dalam ujian pengesahan. Ia mengatasi model lain termasuk 
    XGBoost dan Regresi Logistik.
    """)
